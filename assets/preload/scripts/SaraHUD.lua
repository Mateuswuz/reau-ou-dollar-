...
local pref = {
    ...
    songName = true,
    timer = false,
    iconPoz = false, --icons not moving and on corners of health bar
    hudMiss = true, --purple miss animation for some hud stuff
    HBoverlay = true, --health bar overlay
    timeBarr = true, --vanilla time bar (not a timer)
    ...
    extraInfo = true, --show your sicks, goods, bads and etc.
}
-- true or false guh --
...
function songGuh() --song name
    ...
    if pref.style == 'modern' then
        makeLuaText('song', '' .. (songName), 500, 565, downscroll and 23 or 670)
        setTextAlignment('song', 'left'); 
        setTextSize('song', 20)

        makeLuaSprite('songIcon', 'saraHUD/songIcon', 535, downscroll and 33 or 674)
        setObjectCamera('songIcon', 'hud'); 
        scaleObject('songIcon', 0.5, 0.5)
        addLuaSprite('songIcon')
    end
    addLuaText('song', false);
    setTextFont('song', 'Xolonium-Bold.otf');
    setObjectOrder('song', getObjectOrder('iconP1') - 1)
end

function switchStyle() --styles stuff
    if pref.style == 'classicTxt' then
        setProperty('healthBar.y', 10, downscroll and 68 or 655)
    ...
    if pref.style == 'modern' then
       function onBeatHit()
          ...
          doTweenColor('timeBar', 'timeBar', 'FFFFFF', 0.6, 'linear')
          setProperty('timeTxt.color', getIconColor('dad'))
          doTweenColor('timeTxt', 'timeTxt', 'FFFFFF', 0.6, 'linear')
          setProperty('botplayTxt.color', getIconColor('dad'))
          doTweenColor('botplayTxt', 'botplayTxt', 'FFFFFF', 0.6, 'linear')
        end
        function onStepHit()
        ...
        setTextFont('rating', 'Xolonium-Bold.otf');
        setTextFont('misses', 'Xolonium-Bold.otf');
        setTextFont('score', 'Xolonium-Bold.otf');
...
function timerGuh() --timer
    ...
    setTextFont('timer', 'Xolonium-Bold.otf');
    addLuaText('timer')
end
...
function extraInfoGuh() --extra info, cuz someone needed it
    ...
    setTextFont('sick', 'Xolonium-Bold.otf');
    setTextFont('good', 'Xolonium-Bold.otf');
    setTextFont('bad', 'Xolonium-Bold.otf');
    setTextFont('shit', 'Xolonium-Bold.otf');

    setTextFont('botplayTxt', 'Xolonium-Bold.otf');
    setTextFont('timeTxt', 'Xolonium-Bold.otf');
